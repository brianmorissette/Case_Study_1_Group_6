...
COMMAND="ssh ~/.ssh/student-admin_key -p ${PORT} -o StrictHostKeyChecking=no student-admin@${MACHINE}"
...
scp -i student-admin_key -P ${PORT} -o StrictHostKeyChecking=no ~/.env student-admin@${MACHINE}:~
